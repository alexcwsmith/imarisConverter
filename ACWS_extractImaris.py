...
def _multiSaveIMS(plane, imsFile, channel):
    ...
    if not os.path.exists(os.path.join(directory, 'FullResImages_C' + str(channel)+'/')):
        os.mkdir(os.path.join(directory, 'FullResImages_C' + str(channel)+'/'))
    data = file.get('DataSet/ResolutionLevel 0/TimePoint 0/Channel ' + str(channel)+'/Data')
    imsave(os.path.join(directory, 'FullResImages_C'+str(channel)+'/' + n + '_C' + str(channel) + '_Z' + imNumPadded + '.tif'), np.array(data[plane]), check_contrast=False)
